...
<nav
  class="vana-tabs"
  role="tablist"
  aria-label="<?php echo esc_attr($lang === 'en' ? 'Visit days' : 'Dias da visita'); ?>"
>
  <?php foreach ($days as $i => $day):
    ...
    $label_date = $ts ? wp_date('d/m', $ts) : ($lang === 'en' ? 'Day' : 'Dia') . ' ' . ($i + 1);

    // ── Label do dia da semana (ex: Sáb / Sat) ─────────────
    $label_weekday = '';
    if ($ts) {
      $label_weekday = $lang === 'en'
        ? wp_date('D', $ts)             // Mon, Tue…
        : wp_date('D', $ts);            // Seg, Ter… (locale do WP)
    }
  ...
  ?>
    <a
      ...
      aria-label="<?php echo esc_attr(
        ...
        ($has_live   ? ($lang === 'en' ? ' (Live now)' : ' (Ao vivo)') : '')
      ); ?>"
    >
      <?php if ($tab_label): ?>
